...
-- ifdef LIQUID
data FixedPrim a = FP Int (a -> Ptr Word8 -> IO ())
-- else 
-- data FixedPrim a = FP {-# UNPACK #-} !Int (a -> Ptr Word8 -> IO ())
-- endif

{-@ type PtrGN a N = {v:PtrV a | N <= plen v} @-}

{-@ data FixedPrim a = FP { fpSize :: Nat, fpFill :: (a -> PtrGN Word8 fpSize -> IO ()) } @-}

{-@ fixedPrim :: n:Nat -> (_ -> PtrGN Word8 n -> _) -> _ @-} 
...
{-@ pairF :: fpa:_ -> fpb:_ -> {fpab:_ | size fpab = size fpa + size fpb } @-} 
...
-- #ifdef LIQUID
data BoundedPrim a = BP Int (a -> Ptr Word8 -> IO (Ptr Word8))
-- #else 
-- data BoundedPrim a = BP {-# UNPACK #-} !Int (a -> Ptr Word8 -> IO (Ptr Word8))
-- #endif 

{-@ type PtrS a Base Off = {v:PtrV a | plen v >= (plen Base) - Off } @-}
{-@ data BoundedPrim a = BP { bpSize :: Nat, bpFill :: (a -> p:PtrGN Word8 bpSize -> IO (PtrS Word8 p bpSize)) } @-}
...
-- | @since 0.10.12.0
{-@ boundedPrim :: n:Nat -> (a -> p:PtrGN Word8 n -> IO (PtrS Word8 p n)) -> BoundedPrim a @-}
...
{-# INLINE CONLIKE runB #-}
{-@ runB :: bp:_ -> _ -> p:(PtrGN Word8 {sizeBound bp}) -> IO (PtrS Word8 p (sizeBound bp)) @-}
