...
peekFp fp = unsafeWithForeignPtr fp Data.LiquidPtr.peek 
...
unpackAppendBytesStrict (BS fp len) xs =
    accursedUnutterablePerformIO $ unsafeWithForeignPtr fp $ \base ->
      loop (base `Data.LiquidPtr.plusPtr` (-1)) (base `Data.LiquidPtr.plusPtr` (-1+len)) xs
  where
    loop !sentinal !p acc
      | p == sentinal = return acc
      | otherwise     = do x <- Data.LiquidPtr.peek p
                           loop sentinal (p `Data.LiquidPtr.plusPtr` (-1)) (x:acc)
...
unpackAppendCharsStrict (BS fp len) xs =
  ...
  where
    loop !sentinal !p acc
      | p == sentinal = return acc
      | otherwise     = do x <- Data.LiquidPtr.peek p
                           loop sentinal (p `Data.LiquidPtr.plusPtr` (-1)) (w2c x:acc)
...
wrapAction = flip Data.LiquidPtr.withForeignPtr
